...
v3.8 update:
  - learn_loss_weights=True: clustering weight (gamma) is learned via
    backprop (uncertainty weighting) instead of being a fixed hyperparameter.
  - eta (sentiment weight) stays fixed at 1.0, as before.
  - token attribution stays OFF (plot_token_attribution defaults to False in
    fit(), so it's simply left out of FIT_PARAMS).
"""
...
# gamma dihapus dari sini  diabaikan saat learn_loss_weights=True (dipelajari
# sendiri via backprop). eta tetap fixed = 1.0 sesuai permintaan.
FIT_PARAMS = dict(
    tol                  = 1e-100,
    update_interval      = 10,
    learning_rate        = 1e-3,
    ...
    batch_size           = 256,
    val_ratio            = 0.2,
    # plot_token_attribution tidak ditulis -> default False, token
    # attribution otomatis off, token_attr_bert_name jadi tidak relevan.
)
...
for ds_cfg in DATASETS:
    ...
    for n_clusters in N_CLUSTERS_LIST:
        ...
        # learn_loss_weights=True -> gamma (bobot clustering) dipelajari
        # sendiri via backprop, bukan hyperparameter tetap.
        model = SEMTGPU(dims=DIMS, n_clusters=n_clusters, learn_loss_weights=True)
        ...
        if isinstance(metrics, dict):
            row.update(metrics)
        # catat gamma hasil belajar untuk konfigurasi ini
        row["final_effective_gamma"] = model.get_effective_gamma()
