...
\textbf{Study Program} : Undergraduate Study Program of Statistics
\end{center}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{assets/pics/other/EngFlowchart.png}
    \caption{Schematic diagram of the research framework.}
    \label{fig:ea_workflow}
\end{figure}

% FIXED #2: removed BERTopic-HDBSCAN from baseline list (dropped from evaluation, table 3 no longer includes it)
...
(50{,}000 reviews) and the Yelp Review Dataset
(30{,}000 reviews), both with binary sentiment labels.
...
baseline, MTL-GUI, and BERTopic-KMeans. All
baselines were re-evaluated under identical dataset splits and evaluation
protocols to ensure a fair comparison. Sentiment performance was measured
using accuracy and F1-score; topic quality was assessed using \textit{Topic
Coherence} based on NPMI (TC-NPMI) and \textit{Topic Diversity} (TD).
...
% FIXED #4: "competitive results on IMDB" -> JDST-DEC is actually highest on IMDB too (Table 2)
...
to topic cardinality. Among the compared methods, \textbf{JDST-DEC} achieves
the highest sentiment scores on both IMDB and Yelp across every value of
$K$.
...
% FIXED #2 + #1: removed BERTopic-HDBSCAN mentions; kept BERTopic-KMeans only (matches Table 3)
% FIXED #1: corrected MTL-GUI comparison claim to match Table 3 (JDST-DEC only wins on IMDB TC-NPMI, loses on TD and on Yelp)
% #7 "which is expected" removed
For topic modelling, BERTopic-KMeans achieves the highest TD values across
all configurations (Table~3), reaching 0.9567 on IMDB at $K = 30$. For
TC-NPMI, BERTopic-KMeans also leads across all configurations on IMDB,
while on Yelp, MTL-GUI leads at $K = 30$ and BERTopic-KMeans leads at
$K = 50$ and $K = 80$. \textbf{JDST-DEC} produces lower TC-NPMI and TD
values than BERTopic-KMeans across all configurations, as joint
optimisation prioritises sentiment-discriminative representations over
purely coherent topic structures. Among the joint models, \textbf{JDST-DEC}
outperforms MTL-GUI on TC-NPMI on IMDB, but MTL-GUI achieves higher TD on
IMDB and higher TC-NPMI and TD on Yelp, indicating that the relative
topic quality of the two joint models varies by dataset and metric.
...
\hline
BERTopic-KMeans
  & \cellcolor{green!25}\textbf{0.1566} & \cellcolor{green!25}\textbf{0.9567}
  & \cellcolor{green!25}\textbf{0.1596} & \cellcolor{green!25}\textbf{0.9000}
  & \cellcolor{green!25}\textbf{0.1874} & \cellcolor{green!25}\textbf{0.8413}
  & 0.1077 & \cellcolor{green!25}\textbf{0.7800}
  & \cellcolor{green!25}\textbf{0.1120} & \cellcolor{green!25}\textbf{0.6740}
  & \cellcolor{green!25}\textbf{0.1465} & \cellcolor{green!25}\textbf{0.6438} \\
\hline
MTL-GUI
  & 0.0200 & 0.5833
  & 0.0108 & 0.5800
...
cluster-coherent. Interpretability analysis using occlusion-based token attribution on the
...
% FIXED #3: removed "and produces interpretable topic clusters" from the concluding claim, since interpretability is exploratory (Section 4.7 discussion) and not part of the thesis's stated research questions/objectives, consistent with the Kesimpulan in Bab 5
...
single-task and existing joint-model baselines, without requiring
aspect-level annotations, although the resulting topic representations
remain below the quality achieved by a dedicated topic modelling method
such as BERTopic-KMeans. By showing that a single latent space can serve
both objectives simultaneously, this work opens a practical path toward
richer, annotation-efficient document understanding in large-scale review
