...
baseline, MTL-GUI \parencite{gui2022mtml}, BERTopic-HDBSCAN, BERTopic-KMeans,
and MambaQuin. All baselines were re-evaluated under identical dataset splits
and evaluation protocols to ensure a fair comparison. Sentiment performance
was measured using accuracy and F1-score; topic quality was assessed using
\textit{Topic Coherence} based on NPMI (TC-NPMI) and \textit{Topic Diversity}
(TD).
...
to topic cardinality. MambaQuin records the highest scores on IMDB across
all $K$ settings and is directly comparable on that dataset. Its Yelp
results are not directly comparable due to a different data sampling
procedure (shaded red in Table~2) and are therefore excluded from
column-wise comparison on Yelp. Among the comparable methods,
\textbf{JDST-DEC} achieves the highest sentiment scores on Yelp and the
second-highest on IMDB.
...
\hline
MambaQuin
  & \cellcolor{green!25}\textbf{0.9085} & \cellcolor{green!25}\textbf{0.9085}
  & \cellcolor{green!25}\textbf{0.9083} & \cellcolor{green!25}\textbf{0.9083}
  & \cellcolor{green!25}\textbf{0.9086} & \cellcolor{green!25}\textbf{0.9086}
  & \cellcolor{red!25}0.9433 & \cellcolor{red!25}0.9433
  & \cellcolor{red!25}0.9416 & \cellcolor{red!25}0.9416
  & \cellcolor{red!25}0.9428 & \cellcolor{red!25}0.9428 \\
\hline
\end{tabular}
\end{adjustbox}
\vspace{0.3em}
\noindent{\footnotesize
  \colorbox{green!25}{\,\,} highest comparable value in column;
  \colorbox{red!25}{\,\,} MambaQuin (Yelp) --- not directly comparable
  due to different data sampling;
...
at $K = 30$. For TC-NPMI, MambaQuin leads on IMDB at $K = 30$ (0.1589),
while BERTopic-HDBSCAN and BERTopic-KMeans lead at $K = 50$ and $K = 80$
respectively. On Yelp, BERTopic variants achieve the highest comparable
TC-NPMI values; MambaQuin's Yelp results are excluded from column-wise
comparison due to different data sampling. \textbf{JDST-DEC} produces
lower TC-NPMI and TD values than BERTopic, as joint optimisation
prioritises sentiment-discriminative representations over purely coherent
topic structures. Among the joint models, \textbf{JDST-DEC} consistently
outperforms MTL-GUI on both metrics and both datasets across every value
of $K$.
...
\hline
MambaQuin
  & \cellcolor{green!25}\textbf{0.1589} & 0.2400
  & 0.1561 & 0.2040
  & 0.1606 & 0.1575
  & \cellcolor{red!25}0.2557 & 0.2533
  & \cellcolor{red!25}0.2727 & 0.2060
  & \cellcolor{red!25}0.2699 & 0.1588 \\
\hline
\end{tabular}
\end{adjustbox}
\vspace{0.3em}
