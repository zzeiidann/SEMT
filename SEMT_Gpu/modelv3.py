...
class SEMTGPU(nn.Module):
    ...
    v3.8 Changes (vs v3.7):
       NEW: learn_loss_weights option.
        When True, the clustering loss weight (gamma) is no longer a fixed
        hyperparameter passed to fit(). Instead it becomes a learnable
        parameter optimized jointly with the network via backprop, using
        homoscedastic uncertainty weighting (Kendall, Gal & Cipolla, 2018):

            L = exp(-log_sigma_c) * L_cluster + log_sigma_c
                + eta * L_sentiment

        The `+ log_sigma_c` regularizer is what prevents the trivial
        collapse to weight=0 that a naive learnable multiplicative
        weight would suffer from (since minimizing w*L over w with no
        constraint just drives w -> 0). The sentiment weight (eta) is
        kept fixed as before, exactly as requested.

        `gamma` reparameterized as exp(-log_sigma_c), which is guaranteed
        to lie in (0, +inf) by construction  no extra clamping needed.

        get_effective_gamma() added to inspect the learned weight at any
        point during / after training.

        CSV log gains an `Effective_Gamma` column when learn_loss_weights=True.

    v3.7 Changes (vs v3.6.2), retained:
    ...
    def __init__(
        ...
        # 
        # v3.8: learnable clustering-loss weight (gamma) via uncertainty
        # weighting. Sentiment weight (eta) stays fixed, as requested.
        #
        # gamma_effective = exp(-log_sigma_cluster)  always in (0, +inf).
        # log_sigma_cluster is an ordinary nn.Parameter, so it is picked up
        # automatically by self.parameters() and updated by whatever
        # optimizer fit() builds  no separate optimizer needed.
        # 
        self.learn_loss_weights = bool(learn_loss_weights)
        if self.learn_loss_weights:
            # init at 0.0 -> gamma_effective starts at exp(0) = 1.0
            self.log_sigma_cluster = nn.Parameter(torch.zeros(1))
    ...
    #  v3.8: learnable-weight helper 
    def get_effective_gamma(self) -> Optional[float]:
        """Returns the current effective clustering-loss weight.

        If learn_loss_weights=False, returns None (caller should fall back
        to the `gamma` value passed into fit()).
        """
        if not self.learn_loss_weights:
            return None
        with torch.no_grad():
            return float(torch.exp(-self.log_sigma_cluster).item())
    ...
    def _get_cluster_tfidf_vocab(
        self, all_texts: List[str], cluster_mask: np.ndarray,
        top_n: int = 30, max_features: int = 5000,
    ) -> List[str]:
    ...
    def _occlusion_scores_bidirectional(
        ...
        _SKIP_TOKENS = {"[CLS]","[SEP]","[PAD]","<s>","</s>","<pad>","<mask>"}
    ...
    def compute_token_attribution_per_cluster_with_embeddings(
        ...
        for cid in all_clusters:
            ...
            if neg_indices: neg_mask[neg_indices] = True
            if pos_indices: pos_mask[pos_indices] = True
        ...
        for cid in all_clusters:
            ...
            if pool_neg:
                ranked_neg = self._rank_texts_by_vocab_coverage(
                    [texts[i] for i in pool_neg], vocab_neg[cid], n_neg_quota)
            ...
            if pool_pos:
                ranked_pos = self._rank_texts_by_vocab_coverage(
                    [texts[i] for i in pool_pos], vocab_pos[cid], n_pos_quota)
            ...
            for idx in neg_indices:
                try:
                    ...
                    for tok, sn in zip(toks, sc_neg):
                        ...
                        if clean.lower() not in vset_neg: continue
            ...
            for idx in pos_indices:
                try:
                    ...
                    for tok, sp in zip(toks, sc_pos):
                        if tok in _SKIP_TOKENS: continue
                        clean = self._clean_token(tok)
                        if clean.lower() not in vset_pos: continue
        ...
        for cid in all_clusters:
            ...
            result[cid] = {"neg_pool": neg_agg, "pos_pool": pos_agg}
            print(f"  Cluster {cid:3d}  "
    ...
    def compute_token_attribution_per_cluster(
        ...
        for cid, indices in sorted(cl_idx.items()):
            ...
            ranked  = self._rank_texts_by_vocab_coverage(
                [texts[i] for i in indices], vocab, max_samples_per_cluster)
    ...
    def plot_token_attribution_pos_grid(
        ...
        fig, axes = plt.subplots(nrows, ncols,
                                  figsize=(figsize_per_cluster[0] * ncols,
                                           figsize_per_cluster[1] * nrows),
                                  facecolor="#FAFAFA")
        ...
        for i, cid in enumerate(cids):
            ...
            tokens = list(reversed(
                sorted(pos_d.keys(), key=lambda t: abs(pos_d[t]), reverse=True)[:top_k]))
            vals   = np.array([pos_d[t] for t in tokens])
            ypos   = np.arange(len(tokens))
            max_abs = max(np.abs(vals).max(), 1e-8)
            bar_colors = [plt.cm.Greens(0.35 + 0.55 * (abs(v) / max_abs)) for v in vals]
            ...
            for bar, v in zip(bars, vals):
                if abs(v) > 1e-6:
                    ax.text(v + max_abs * 0.02, bar.get_y() + bar.get_height() / 2,
    ...
    def plot_token_attribution_neg_grid(
        ...
        for i, cid in enumerate(cids):
            ...
            if not neg_d:
                ax.text(0.5, 0.5, "No negative data", ha="center", va="center",
                        transform=ax.transAxes, color="#AAAAAA", fontsize=9)
                ax.axis("off"); continue

            tokens = list(reversed(
                sorted(neg_d.keys(), key=lambda t: abs(neg_d[t]), reverse=True)[:top_k]))
            vals   = np.array([neg_d[t] for t in tokens])
            ypos   = np.arange(len(tokens))
            max_abs = max(np.abs(vals).max(), 1e-8)
            bar_colors = [plt.cm.Reds(0.35 + 0.55 * (abs(v) / max_abs)) for v in vals]
    ...
    def plot_token_attribution_per_cluster(
        ...
        for i, cid in enumerate(cids):
            ...
            all_tokens = list(reversed(list(dict.fromkeys(neg_tokens + pos_tokens))))
    ...
    def plot_token_attribution_heatmap(
        ...
        top_pos_tokens = [
            t for t, _ in sorted(
                ...
                key=lambda x: x[1], reverse=True)[:top_k_global]
        ]
        top_neg_tokens = [
            t for t, _ in sorted(
    ...
    def explain_single_text(
        ...
        for tok, sp, sn in zip(tokens, scores_pos, scores_neg):
            ...
            dt.append(clean); ds_pos.append(sp); ds_neg.append(sn)

        dt     = np.array(dt)
        ds_pos = np.array(ds_pos)
        ds_neg = np.array(ds_neg)
        order  = np.argsort(np.abs(ds_pos) + np.abs(ds_neg))[::-1][:20][::-1]
    ...
    def plot_cluster_evolution(
        ...
        for i, cid in enumerate(uniq):
            ...
            ax.scatter(emb_2d[mask, 0], emb_2d[mask, 1],
                       c=[colors[i]], marker="o", s=point_size, alpha=alpha,
    ...
    def fit(
        self,
        dataset,
        gamma: float = 1.0,       # clustering KL-divergence weight (ignored if learn_loss_weights=True)
        eta:   float = 0.1,       # sentiment cross-entropy weight (ALWAYS fixed, never learned)
        ...
        """
        Joint DEC + Sentiment training  (v3.8).

        The encoder is updated purely by:
            L_cluster   (KL divergence to target distribution, DEC-style)
            L_sentiment  (cross-entropy, when labels are available)
        ...
        been removed (since v3.7).

        v3.8: if self.learn_loss_weights is True (set at __init__ time), the
        `gamma` argument passed here is IGNORED for the actual loss
        computation  the clustering weight is instead learned jointly via
        backprop using homoscedastic uncertainty weighting:

            L = exp(-log_sigma_c) * L_cluster + log_sigma_c + eta * L_sentiment

        `eta` (sentiment weight) always stays fixed at whatever value you
        pass in  it is never learned, exactly as requested.
        ...
        """
        print("=" * 60)
        print("SEMTGPU v3.8  Joint Training: Clustering + Sentiment")
        print("  (reconstruction loss removed  lives in pretrain_autoencoder)")
        if self.learn_loss_weights:
            print(f"Loss weights: (cluster)=LEARNED via backprop (uncertainty weighting), "
                  f"(sentiment)={eta} [fixed]")
            print(f"  initial effective gamma = {self.get_effective_gamma():.4f}")
        else:
            print(f"Loss weights: (cluster)={gamma} [fixed], (sentiment)={eta} [fixed]")
        ...
        if plot_token_attribution:
            ...
            print(f"  token filter     = VOCAB-CONSTRAINED")
        ...
        # NOTE: self.parameters() already includes log_sigma_cluster (if
        # learn_loss_weights=True) since it was registered as nn.Parameter
        # in __init__  no separate optimizer / param group needed.
        ...
        log_fields = [
            ...
            "L", "Lc", "Ls",                               # Lr removed
            "Effective_Gamma",                             # v3.8: learned or fixed gamma
        ...
        with open(log_path, "w", newline="") as logfile:
            ...
            for ite in range(maxiter):
                ...
                if ite % update_interval == 0:
                    ...
                    eff_gamma = self.get_effective_gamma()
                    eff_gamma_display = eff_gamma if eff_gamma is not None else gamma
                    ...
                    print(f"\nIter {ite:5d} | Lc={avg_Lc:.5f}  Ls={avg_Ls:.5f}  L={avg_L:.5f}  "
                          f"gamma_eff={eff_gamma_display:.5f}")
                    ...
                    writer.writerow({
                        ...
                        "Lc": round(avg_Lc, 5),
                        "Ls": round(avg_Ls, 5),
                        "Effective_Gamma": round(eff_gamma_display, 5),
                    ...
                    # FIX 4: only trigger early stop after at least one full update interval
                    if ite >= update_interval and delta < tol:
                        print(f"  label={delta:.6f} < tol={tol}  early stop.")
                        break
                ...
                for batch in tqdm(train_loader, desc=f"Train {ite}", leave=False):
                    ...
                    # Forward: encoder only  no decode(), no reconstruction loss
                    ...
                    if self.learn_loss_weights:
                        # v3.8: uncertainty weighting  gamma learned via backprop,
                        # eta stays fixed as requested. The "+ log_sigma_cluster"
                        # term is a regularizer that prevents the trivial
                        # collapse to weight=0 that a naive learnable
                        # multiplicative weight would produce.
                        precision_c = torch.exp(-self.log_sigma_cluster)
                        loss = (precision_c * cl + self.log_sigma_cluster).squeeze() \
                               + eta * sl
                    else:
                        # v3.7 behaviour, unchanged
        ...
        if self.learn_loss_weights:
            print(f"Final learned effective gamma: {self.get_effective_gamma():.5f}")
        ...
        best_summary = {
            ...
            "n_train":        n_train,
            "n_val":          n_val,
            "learn_loss_weights": self.learn_loss_weights,
            "final_effective_gamma": self.get_effective_gamma(),
        }
        ...
        with torch.no_grad():
            ...
            q_all_f2 = torch.cat(qs, 0); s_all_f = torch.cat(ss, 0)
        y_pred_cl   = q_all_f2.argmax(1).cpu().numpy()
        ...
        if plot_token_attribution and has_texts:
            try:
                print(f"\n[Token Attribution  Final Best Model  (v3.8)]")
